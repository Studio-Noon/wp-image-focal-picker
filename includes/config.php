...
if ( ! function_exists( 'noon_focal_secret_file' ) ) {
	...
	function noon_focal_ensure_secret() {
		...
		if ( ! file_exists( $file ) ) {
			...
			// A PHP file that only defines a constant: requesting it directly returns nothing.
			if ( false === @file_put_contents( $file, $contents, LOCK_EX ) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents, WordPress.PHP.NoSilencedErrors.Discouraged -- must work before WP_Filesystem credentials exist; failure is reported to the admin.
				return false;
			}
